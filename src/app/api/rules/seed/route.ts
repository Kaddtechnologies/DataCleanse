...
export async function POST(request: NextRequest) {
  try {
    console.log('🌱 Starting database seeding for predefined business rules...');

    // Check if rules already exist to avoid duplicates
    const existingRules = await db.query(
      'SELECT COUNT(*) as count FROM business_rules WHERE id IN ($1, $2, $3)',
      [
    ...
    if (parseInt(existingRules.rows[0].count) > 0) {
      return NextResponse.json({
        success: true,
        message: 'Predefined rules already exist',
        seeded: false
      });
    }

    // Clear any existing predefined rules first (safety measure)
    await db.query(
      `DELETE FROM business_rules WHERE id IN ($1, $2, $3)`,
      [
        'joint-venture-detection-001',
        'energy-division-legitimacy-002', 
        'freight-forwarder-exemption-003'
    ...
    await db.query(`
      INSERT INTO business_rules (
        id, name, description, author, rule_type, priority, accuracy, status, enabled, version,
        rule_code, metadata, config, ai_generated, execution_count, total_execution_time, 
        avg_execution_time, created_at, updated_at, last_executed_at
...
function evaluateFreightForwarder(record1, record2) {
  ...
  return { action: "continue_evaluation" };
    ...
    `, [
      // Joint Venture Test Case
      'jv-test-001', 'joint-venture-detection-001', 'Ruhr Oel JV - BP vs Rosneft', 'Real production example of BP/Rosneft joint venture',
      JSON.stringify({"customer_name": "Ruhr Oel GmbH - BP Europa SE", "address": "Industriepark Schwechat, Austria"}),
      JSON.stringify({"customer_name": "Ruhr Oel GmbH - Rosneft Oil Company", "address": "Industriepark Schwechat, Austria"}),
      JSON.stringify({"shouldMatch": false, "confidence": "high", "score": 95, "recommendation": "reject", "reasoning": "Different parent companies in joint venture"}),
      'standard', 'AI System',
      
      // Energy Division Test Case
      'energy-test-001', 'energy-division-legitimacy-002', 'ExxonMobil Oil vs Chemical', 'ExxonMobil divisions serving different markets',
      JSON.stringify({"customer_name": "ExxonMobil Oil Company", "address": "5959 Las Colinas Boulevard, Irving, TX", "industry": "Oil & Gas"}),
      JSON.stringify({"customer_name": "ExxonMobil Chemical Company", "address": "5959 Las Colinas Boulevard, Irving, TX", "industry": "Chemical Manufacturing"}),
      JSON.stringify({"shouldMatch": false, "confidence": "high", "score": 95, "recommendation": "reject", "reasoning": "Legitimate business divisions"}),
      'standard', 'AI System',
      
      // Freight Forwarder Test Case
      'freight-test-001', 'freight-forwarder-exemption-003', 'DHL at Customer Address', 'Freight forwarder at customer shipping location',
      JSON.stringify({"customer_name": "Acme Chemical Manufacturing", "address": "123 Industrial Way, Houston, TX", "sic_code": "2869"}),
      JSON.stringify({"customer_name": "DHL Express", "address": "123 Industrial Way, Houston, TX", "sic_code": "4215"}),
      JSON.stringify({"shouldMatch": false, "confidence": "high", "score": 95, "recommendation": "reject", "reasoning": "Freight forwarder vs actual customer"}),
      'standard', 'AI System'
    ]);

    console.log('✅ Successfully seeded 3 predefined business rules with test cases');

    return NextResponse.json({
      success: true,
      message: 'Successfully seeded 3 predefined business rules for demo!',
      seeded: true,
      rules: [
        'joint-venture-detection-001',
        'energy-division-legitimacy-002',
    ...
    console.error('❌ Error seeding predefined rules:', error);
    return NextResponse.json(
      { 
        error: 'Failed to seed predefined rules', 
